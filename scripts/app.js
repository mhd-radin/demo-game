...
let app = {
  ...
  use: {
    ...
    loader(
      ...
      if (pathArr.length == 0)
        throw "ERROR: parameter 'pathArr' is undefined :: 'pathArr' is empty array or undefined ";
      var loadIndex = 0;
      ...
      function load(path) {
        app.connectScript(path).onload = function () {
          ...
          loadIndex += 1;
          if (
            opt.onloadfile instanceof Function ||
            typeof opt.onloadfile == "function"
          )
            opt.onloadfile({
              path,
              loadIndex
          ...
          if (loadIndex == pathArr.length - 1) {
            // finshid
            if (
              opt.onfinish instanceof Function ||
              typeof opt.onfinish == "function"
            )
              opt.onfinish();
          }
        };
    ...
    app.connectScript(ws.PATH_CLASSES + "common_class.js").onload = () => {
      app.connectScript(ws.PATH_CLASSES + "player.js").onload = () => {
        app.connectScript(ws.PATH_PAGES + "home.js").onload = function () {
          app.connectScript("scripts/methods/connectCommonScripts.js").onload =
            function () {
              app.init();
              ws.SCRIPT_HOME_PAGE.run();
            };
